...
def euler(f, y0, t0, T, h):
    ...
    while t < T:                         #Tant que la date t est inférieure à la date T
        ...
        t_list.append(t)                  #on ajoute cette nouvelle date à la fin de la liste des dates
...
def euler_vect(F, Y0, t0, T, h):
    ...
    t_list = [t0]                                       #initialisation d'une liste avec les valeurs de t
    Y_list = [Y0]                                       #initialisation d'une liste avec les valeurs de y
    t = t0                                              #initialisation de t à t0, la borne inf 
                                                        #de l'intervalle de résolution
    Y = Y0                                              #initialisation de y à y0, la condition initiale

    while t < T:                                        #Tant que la date t est inférieure à la date T
        t = t + h                                       #on incrémente la date avec le pas de calcul h
        t_list.append(t)                                #on ajoute cette nouvelle date à la fin de la liste des dates 
                                                        #(round = arrondi à "digits" décimales)
...
def rk4_vect(F, Y0, t0, T, h) :
    ...
    while t < T:                         #Tant que la date t est inférieure à la date T
        ...
        Y = [Y[i] + (h/6)*(k1[i] + 2*k2[i] + 2*k3[i] + k4[i]) for i in range(len(Y))]   #on calcule la nouvelle valeur de Y
