...
def runge_kutta_4(f, y0, t0, T, n, digits=10) :
"""
...
Paramètres
    ...
    while t < T:                                        #Tant que la date t est inférieure à la date T
        ...
        Y = [ y + h/6*(k1+2*k2+2*k3+k4) for y, k1 in zip(Y, F(t, Y)), k2 in zip(Y, F(t+h/2, Y+k1*h/2)), k3 in zip(Y, F(t+h/2, Y+k2*h/2)),k3 in zip(Y, F(t+h/2, Y+k2*h/2)), k4 in zip(Y, F(t+h/2, Y+h*k3)) ] #on calcule la nouvelle valeur de y
        Y_list.append(Y)                                #on ajoute le nouveau Y à la fin de la liste des y
